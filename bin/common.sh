...
case "x$KERNEL" in
    "xSunOS")
		...
		SOLARIS_10=1
		SOLARIS_11=0
		# not going to search gdate and gawk, freeware on s10
		PATH=$PATH:solaris10/`uname -p`
		GAWK=solaris10/`uname -p`/gawk
		export PATH
		LANG=
        	AWK=nawk
	else
        	echo "UNIX flavor [$KERNEL] unsupported for Splunk solaris11 App, quitting" > $TEE_DEST
		exit 1	
	fi	
        ;;
    *)
